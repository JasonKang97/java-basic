...
// 1byte(8bit) 단위로 데이터 파일 입출력 - 보통 클래스의 이름에 stream이 포함됨.
// 문자, 그림, 미디어 등 다양한 종류의 데이터 처리가 가능.
public class Ex40FileIo {
	public void writeFile(File file, ArrayList<String> list) {
		try {
			BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), "utf-8"));	// "utf-8" >>> 한글이 꺠질 때 사용가능.
			for(String ss:list) {
	...
	public void readFile(File file) {
		try {
			...
			// String ss = "kor' + "eng"; >>> 자바는 문자열 더하기 비권장: 메모리 낭비가 심함. String 은 불변으로 매번 객체가 생성됨.
