...
public class DbTest1 {
	...
	public DbTest1() {
		...
		try {
			...
			rs.next();	// record pointer 이동 명령어: pointer가 가리키고 있는 record만 참조 가능. 기본적으로 첫 record보다 위에 위치함.
			...
			*/
			...
			while(rs.next()) { // 자료가 있는 동안 반복
				String code =rs.getString("code"); 
				String sangpum =rs.getString("sang"); 
				int su =rs.getInt(3);	// DB에서 불러올 때 순서가 매겨짐. 즉 현재 순서는 1은 code, 2는 sang, 3은 su, 4는 dan이다. 순서(index)를 적을 수 있다.
