...
public class MyStream1 {
	...
	private void test1() {
		...
		// Collection 인터페이스는 stream()이 정의되어 있기 때문에 Collection을 구현한 객체(List, Set, Map 등)는 이를 이용해 Stream 생성.
		...
		Stream<String> stream4 = Arrays.stream(new String[] {"a", "b", "c"},0,3);	// 시작 인덱스 0부터 시작하여 끝 인덱스(3) 바로 전까지의 배열 요소를 포함한 스트림 반환.
		
		// 원시 Stream 생성하기: 원시 자료형을 대상으로 함.
		IntStream iStream = IntStream.range(5, 10);
		int hap = 0;
		iStream.forEach(para -> System.out.print(para + " "));
		System.out.println();
	...
	private void test2() {
		List<String> list = Arrays.asList("레밍스", "팩맨", "마리오"); // 배열을 List화 한 것.
		...
		System.out.println();
		for(String ss: list) {
			System.out.println(ss);	// 전통적인 방법: 향상된 for문 사용
		}
		
		System.out.println();
		Stream<String> stream = list.stream(); //  Collection으로부터 Stream 객체 얻기(람다로 처리 가능한 반복자 역할)
		stream.forEach(str -> System.out.println(str)); // 내부 반복으로 순차 처리됨.
		//stream.forEach(str -> System.out.println(str)); // stream has already been operated upon or closed >>> Stream은 1회용 객체이므로 한번 더 사용 불가
		...
		// peek(): 스트림의 각 요소에 대해 지정된 작업을 수행하고 요소를 변경하지 않은 채 다음 작업으로 전달(보류의 개념). 다양한 작업을 한 번의 스트림에서 연속적으로 체이닝하여 수행할 경우에 사용함.
		
		list.stream().peek(System.out::println).forEach(System.out::println);
		
		System.out.println();
		System.out.println("병렬 처리");
		// parallelStream(): Collection의 Stream을 병렬 모드로 처리. 대량의 자료 를 출력할 때 유용.
		// 내부적으로 thread를 사용함. 작업 처리 시간을 단축 가능.
	...
	private void test3() {
		System.out.println("\nStudent type의 DTO(Data Transfer Object)클래스로 컬렉션 작성");
		...
		// stream 활용: 나이 평균 출력1
		double avg = slist.stream().mapToInt(Student::getAge)// Student 객체를 age값으로 매핑
					.average()	// age 요소의 평균을 OptionalDouble 타입(null관련 에러를 회피)으로 저장.
					.getAsDouble();
		...
		// 필터(filter) 처리
		double avgResult = slist.stream().filter(d -> d.getGender().equals("남")).mapToDouble(Student::getAge).average().getAsDouble();
